...
async function fetchConfigStatus() {
    ...
    try {
        ...
        if (data.status === 'present') {
            banner.className = 'mb-10 p-6 glass status-banner flex items-center gap-5 border-emerald-500/20';
            icon.innerText = '✅';
            title.innerText = 'Connected';
            message.innerText = `Encryption key is set (${data.format}).`;
            guidance.innerText = '';
        } else {
            banner.className = 'mb-10 p-6 glass status-banner flex items-center gap-5 border-red-500/20';
            icon.innerText = '❌';
