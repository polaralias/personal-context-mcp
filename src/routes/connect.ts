...
<body class="bg-gray-100 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white rounded-lg shadow-lg max-w-lg w-full p-8">
        <h1 class="text-2xl font-bold text-gray-800 mb-2 text-center">${configSchema.name}</h1>
        <p class="text-gray-600 text-center mb-6">${configSchema.description}</p>
        ...
        <form action="/connect" method="POST" class="space-y-4">
            ...
            <fieldset class="${missingParams.length > 0 ? 'opacity-50 cursor-not-allowed' : ''}" ${missingParams.length > 0 ? 'disabled' : ''}>
                ...
                <div class="pt-4">
                    <button type="submit" 
                        class="w-full bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded font-medium transition ${missingParams.length > 0 ? 'opacity-50 cursor-not-allowed' : ''}"
