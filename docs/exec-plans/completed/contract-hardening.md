...
Turn validated findings into explicit behavior contracts and then enforce them in code and tests.
...
- tool input and output normalization

## Contract decisions now fixed in docs

- scheduled location remains part of the desired product contract and must become real resolver behavior unless the product spec is explicitly changed first
...
- scheduled location is real resolver behavior
...
- scheduled-location input is normalized and validated at the public write boundary
...
- specs exist
- docs match implemented tested behavior
